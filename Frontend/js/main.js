...
function statCard(label, value, color, bg, iconPath, extra = '') {
  return `<div class="card stat-card" style="padding:20px; border-left: 4px solid ${color};">
...
// ===== AUDIT LOG =====
window.logActivity = async function (actionType, formId, formName, detail, targetType) {
  try {
    ...
    await fetch(`${API_BASE}/audit-logs`, {
      ...
      body: JSON.stringify({ actionType, formId, formName, detail, targetType })
    });
  } catch (err) {
