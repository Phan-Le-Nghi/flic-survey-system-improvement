...
const _bellBg = { info:'#00008B', success:'#dcfce7', warning:'#fef9c3', error:'#fee2e2' };
  ...
  body.innerHTML = notifs.length
    ? notifs.map(n => {
        ...
        return `<div class="notif-item${n._bellNew?' unread':''}">
          ...
          <div><div class="notif-title">${n.title||''}</div><div class="notif-msg">${n.msg||''}</div><div class="notif-time">${getBellApprovalDate(n)}</div></div>
        </div>`;
      }).join('')
