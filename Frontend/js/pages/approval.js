...
async function confirmApprove() {
  ...
  if (typeof logActivityAction === 'function') {
    logActivityAction('approve', 'Phê duyệt', item.form, 'Chuyển trạng thái từ Chờ duyệt sang Đã duyệt', item.form_id || item.formId);
  }
...
function sendApproveNotification(item) {
  ...
  try {
    ...
    fetch(`${API_BASE}/notifications`, {
      ...
      headers: getAuthHeaders(),
      body: JSON.stringify({
        tieu_de: newNotif.title,
        noi_dung: newNotif.msg,
        loai: newNotif.type,
        nguoi_nhan: newNotif.recipients,
        trang_thai: newNotif.status,
        ngay_gui: new Date().toISOString(),
        tong_nguoi_nhan: newNotif.total,
...
async function confirmReject() {
  ...
  if (typeof logActivityAction === 'function') {
    logActivityAction('reject', 'Từ chối', item.form, `Từ chối phê duyệt biểu mẫu (Lý do: ${reason})`, item.form_id || item.formId);
  }
