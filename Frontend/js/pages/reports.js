...
const API = API_BASE;
const BLUE='#00008B', RED='#ef4444', PALETTE=['#00008B', '#ea580c', '#3b82f6', '#f97316', '#0d9488', '#0284c7', '#d97706', '#6366f1', '#fb923c', '#0ea5e9'];
  ...
  if (cfg.type === 'bar' && cfg.data?.datasets) {
    cfg.data.datasets.forEach(ds => {
      ...
      if (ds.maxBarThickness === undefined) {
        ds.maxBarThickness = 80; // Allow bars to be slightly wider but still capped
      }
    });
  }
...
<div id="rpt-picker">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    <div class="dash-card">
      <div class="dash-card-header" style="align-items: center;">
        ...
        <div style="display:flex; gap: 8px; align-items:center;">
          <select id="rpt-form-category-filter" onchange="renderFormListWithImports()" style="padding: 6px 10px; border-radius: 8px; border: 1px solid var(--gray-200); font-size: 12px; color: var(--gray-700); background: var(--gray-50); cursor: pointer; outline: none; font-weight: 600;">
            <option value="">Tất cả danh mục</option>
            <option value="Tin học">Tin học</option>
            <option value="Ngoại ngữ">Ngoại ngữ</option>
          </select>
          <div style="width:36px;height:36px;border-radius:10px;background:#00008B;display:flex;align-items:center;justify-content:center;cursor:pointer;" title="Làm mới" onclick="loadFormList()">
            <svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" width="18" height="18"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          </div>
        </div>
      </div>
    ...
    <div class="dash-card" style="border:2px dashed #e2e8f0;cursor:pointer;box-shadow:none"
         ...
         onmouseenter="this.style.borderColor='var(--brand-color)';this.style.background='var(--gray-100)'"
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:220px;padding:32px;text-align:center">
        ...
        <div style="font-size:16px;font-weight:700;color:var(--gray-900);margin-bottom:6px">Import file Excel</div>
        <div style="font-size:12.5px;color:#94a3b8;margin-bottom:4px">Hỗ trợ .xlsx, .xls</div>
        <div style="font-size:12px;color:var(--brand-color);font-weight:600">Bấm để chọn file</div>
...
function _showPeopleModal(title, sub, people) {
  ...
  modal.id = 'rating-people-modal';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.45);z-index:9999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(3px)';
  const colors = [BLUE,'#0284c7','#059669','#d97706',RED,'#00008B'];
...
function renderReportTextAnswers(qId, keyword = '') {
  ...
  const colors = ['#00008B', '#ea580c', '#3b82f6', '#10b981', '#8b5cf6', '#eab308', '#ef4444', '#14b8a6'];
...
function drawQuestionDetailCharts() {
  (analysisData?.questions || []).forEach(q => {
    ...
    if (['choice','checkbox','dropdown'].includes(type)) {
      ...
      if (ratingContainer) {
        const ratingColors = ['#ef4444', '#f97316', '#60a5fa', '#2563eb', '#00008B'];
        let barsHtml = '';
        for (let i = 4; i >= 0; i--) {
