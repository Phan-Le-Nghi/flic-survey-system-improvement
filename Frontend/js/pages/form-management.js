...
function moveToTrash(form, reason) {
  const trash = loadTrash();
  const deletedAt = Date.now();
  const purged = trash.filter(f => f.expiresAt > Date.now());
  purged.unshift({ ...form, deletedAt, expiresAt: deletedAt + 30 * 24 * 60 * 60 * 1000, deleteReason: reason || 'Không có lý do' });
  saveTrash(purged);
}
...
async function submitForm(skipCloseConfirm = false, options = {}) {
  ...
  if (typeof logActivityAction === 'function') {
    logActivityAction('create', 'Tạo mới', name, isAutoDraft ? 'Tạo bản nháp tự động' : 'Tạo biểu mẫu mới', newId);
  }
...
async function deleteForm(id) {
  ...
  const reason = prompt(`Chuyển biểu mẫu "${f.name}" vào thùng rác?\n\nNhập lý do xóa (nếu có):\n(Biểu mẫu sẽ tự động xóa vĩnh viễn sau 30 ngày)`);
  ...
  if (typeof logActivityAction === 'function') {
    logActivityAction('delete', 'Xóa', f.name, `Đưa biểu mẫu vào thùng rác (Lý do: ${reason.trim() || 'Không có'})`, f.id);
  }
...
async function duplicateForm(id) {
  ...
  try {
    ...
    if (typeof logActivityAction === 'function') {
      const newForm = FORMS.find(x => x.name === copyName);
      logActivityAction('create', 'Tạo mới', copyName, 'Tạo bản sao từ biểu mẫu: ' + f.name, newForm ? newForm.id : '');
    }
  } catch (e) {}
...
async function saveEditForm() {
  ...
  if (typeof logActivityAction === 'function') {
    logActivityAction('edit', 'Chỉnh sửa', name, 'Cập nhật nội dung biểu mẫu', id);
  }
