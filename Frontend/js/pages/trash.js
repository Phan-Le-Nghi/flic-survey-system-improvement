      ...
      <div class="card-body" style="padding:14px 20px">
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
          <div class="input-wrap" style="flex:1;min-width:220px">
            ...
            <input type="text" class="input" placeholder="Tìm kiếm biểu mẫu trong thùng rác..." style="background:#f8fafc" id="trash-search-input" oninput="handleTrashSearch(this.value)">
          </div>
    ...
    return `
              <tr style="border-bottom:1px solid #f1f5f9;transition:all 0.2s;${isChecked ? 'background:#eff6ff' : ''}" onmouseenter="if(!${isChecked})this.style.background='#f8fafc'" onmouseleave="if(!${isChecked})this.style.background='transparent'">
                ...
                <td style="padding:16px 20px;">
                  ...
                  <div style="display:inline-block;padding:2px 8px;border-radius:6px;background:${catBg};color:${catColor};font-size:11.5px;font-weight:600;">${f.cat || 'Chưa phân loại'}</div>
                </td>
                <td style="padding:16px 20px;">
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal" onclick="event.stopPropagation()" style="max-width:400px;border-radius:14px">
        ...
        <div id="trash-restore-msg" style="padding:20px;font-size:14px;color:var(--gray-700);text-align:left">
        </div>
...
async function confirmTrashRestore() {
  ...
  try {
    await fetch(`${API_BASE}/forms/${target.id}/restore`, { method: 'PATCH', headers: authHeaders() });
    await loadTrashData();
    trashSelectedIds.delete(String(target.id));
    updateTrashActionButtons();

    if (typeof logActivityAction === 'function') {
      logActivityAction('restore', 'Khôi phục', target.name, 'Khôi phục biểu mẫu từ thùng rác', target.id);
    }
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal" onclick="event.stopPropagation()" style="max-width:400px;border-radius:14px">
        ...
        <div id="trash-delete-msg" style="padding:20px;font-size:14px;color:var(--gray-700);text-align:left">
...
async function confirmTrashDelete() {
  ...
  if (target.type === 'single') {
    try {
      await fetch(`${API_BASE}/forms/${target.id}/permanent`, { method: 'DELETE', headers: authHeaders() });
      ...
      if (typeof logActivityAction === 'function') {
        logActivityAction('delete', 'Xóa vĩnh viễn', target.name, 'Xóa vĩnh viễn biểu mẫu khỏi hệ thống', target.id);
      }
