      ...
      <tr style="border-bottom:${borderB}; transition:background 0.2s" onmouseenter="this.style.background='#f8fafc'" onmouseleave="this.style.background='none'">
        ...
        <td style="padding:16px 20px;">
          <div style="display:flex; align-items:center; gap:12px;">
            ${f.avatar && !isAnon ? `
              <img src="${f.avatar}" alt="Avatar" style="width:36px; height:36px; border-radius:50%; border:2px solid #3b0a99; object-fit:cover; flex-shrink:0;" onerror="this.onerror=null; this.outerHTML='<div style=\\'width:36px; height:36px; border-radius:50%; background:${avatarColor.bg}; color:${avatarColor.color}; border:2px solid #3b0a99; -webkit-print-color-adjust: exact; print-color-adjust: exact; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:16px; flex-shrink:0; box-sizing:border-box;\\'>${initial}</div>'">
            ` : `
  ...
  fetch(`${API}/feedback/${id}/chitiet`, { headers: authHeaders() })
    ...
    .then(data => {
      ...
      container.innerHTML = `
        ...
        <!-- Breadcrumb & Header -->
        <div class="no-print" style="margin-bottom:24px; display:flex; align-items:flex-start; justify-content:space-between;">
          <div>
        ...
        <div id="detail-print-area" style="display:flex; gap:24px; align-items:flex-start;">
          <div style="width:340px; flex-shrink:0; display:flex; flex-direction:column; gap:24px; position:sticky; top:24px;">
            <div class="print-border-0" style="background:#fff; border-radius:12px; border:1px solid var(--gray-200); padding:24px; box-shadow:var(--shadow-sm);">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
                ...
                ${f.avatar && !isAnon ? `
                  <img src="${f.avatar}" alt="Avatar" style="width:40px; height:40px; border-radius:50%; border:2px solid #3b0a99; object-fit:cover; flex-shrink:0;" onerror="this.onerror=null; this.outerHTML='<div style=\\'width:40px; height:40px; border-radius:50%; background:${avatarColors.bg}; color:${avatarColors.color}; display:flex; align-items:center; justify-content:center; font-size:15px; font-weight:800; border:2px solid #3b0a99; -webkit-print-color-adjust:exact; print-color-adjust:exact;\\'>${initials}</div>'">
                ` : `
...
function deleteFeedback(id) {
  const reason = prompt('Chuyển phản hồi này vào thùng rác?\n\nNhập lý do xóa (nếu có):');
  ...
  fetch(`${API}/feedback/${id}`, {
    ...
    if (res.ok) {
      if (typeof showToast === 'function') showToast('Đã chuyển phản hồi vào thùng rác', 'success');
      history.back();
    } else {
...
function deleteAllFeedbacks() {
  ...
  const reason = prompt(`Chuyển ${checked.length} phản hồi đã chọn vào thùng rác?\n\nNhập lý do xóa (nếu có):`);
  if (reason === null) return;
  
  // Basic loop for deleting, ideal would be a batch delete API
  Promise.all(checked.map(id => 
    ...
    if (typeof showToast === 'function') showToast('Đã chuyển các phản hồi được chọn vào thùng rác', 'success');
    fetchFeedbacks();
  }).catch(err => console.error(err));
}
