...
router.get("/:nhan_vien_id", async (req, res) => {
  try {
    const result = await sql.query`
      SELECT yc.id, yc.form_id, yc.ngay_them,
             f.ten_form, f.danh_muc, f.trang_thai, f.luot_xem,
      ...
      GROUP BY yc.id, yc.form_id, yc.ngay_them, f.ten_form, f.danh_muc, f.trang_thai, f.luot_xem, n.ho_ten
      ORDER BY yc.ngay_them DESC
    `;
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (check.recordset.length > 0)
      return res.status(409).json({ message: "Form đã được yêu thích" });
